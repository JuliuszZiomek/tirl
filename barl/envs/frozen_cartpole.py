...
class FrozenCartpoleEnv(gym.Env):
    ...
    def __init__(self, use_trig=False):
        ...
        self.l = FrozenCartpoleEnv.POLE_LENGTH  # pole's length
...
def get_pole_pos(x):
    xpos = x[..., 0]
    theta = x[..., 2]
    pole_x = FrozenCartpoleEnv.POLE_LENGTH * np.sin(theta)
    pole_y = FrozenCartpoleEnv.POLE_LENGTH * np.cos(theta)
...
def pilco_cartpole_reward(x, next_obs):
    ...
    goal = np.array([0.0, FrozenCartpoleEnv.POLE_LENGTH])
...
def tf_get_pole_pos(x):
    xpos = x[..., 0]
    theta = x[..., 2]
    pole_x = FrozenCartpoleEnv.POLE_LENGTH * tf.sin(theta)
    pole_y = FrozenCartpoleEnv.POLE_LENGTH * tf.cos(theta)
...
def tf_pilco_cartpole_reward(x, next_obs):
    ...
    goal = tf.constant([0.0, FrozenCartpoleEnv.POLE_LENGTH], dtype=tf.float32)
